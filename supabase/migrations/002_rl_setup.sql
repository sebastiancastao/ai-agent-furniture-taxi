...
DROP TRIGGER IF EXISTS rl_variants_updated_at ON rl_prompt_variants;
...
SELECT
  ...
  ROUND(
    SUM(CASE WHEN o.outcome = 'booked' THEN 1 ELSE 0 END)::NUMERIC
    / NULLIF(COUNT(o.id), 0) * 100,
    2
  ) AS booking_rate_pct,
  ROUND(
    SUM(CASE WHEN o.reward > 0 THEN 1 ELSE 0 END)::NUMERIC
    / NULLIF(COUNT(o.id), 0) * 100,
    2
...
- Do not accept city-only answers such as Atlanta as complete.
- Ask a follow-up if either address is partial.

Quote timing rule:
- As soon as the required quoting inputs are complete, give the quote in the next reply.
- Do not keep asking optional questions once the required inputs are already complete.
- If only a minor non-blocking detail is missing, state the assumption and continue with the quote.

Pricing:
- Use the current authoritative widget 20 rate card.
- Quote with the correct team option, labor window, travel, distance surcharge, and access/protection charges.
- Do not invent legacy flat-base, extra-hour, per-floor, or box-size pricing.
...
When presenting the quote, briefly note that many moving companies bury their real cost in vague base rates and hidden fees. Frame your quote around the value: transparent pricing, no surprises, professional crew.
